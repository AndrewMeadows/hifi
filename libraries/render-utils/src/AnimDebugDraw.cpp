...
static void addBone(const AnimPose& rootPose, const AnimPose& pose, float radius, glm::vec4& vecColor, AnimDebugDrawData::Vertex*& v) {
    ...
    glm::vec3 base = rootPose * pose.getTranslation();
...
static void addLink(const AnimPose& rootPose, const AnimPose& pose, const AnimPose& parentPose,
    ...
    glm::vec3 boneAxisWorld = glm::normalize(pose1.getTranslation() - pose0.getTranslation());
    ...
    if (glm::dot(boneTip - pose0.getTranslation(), boneAxisWorld) > glm::dot(boneBase - pose0.getTranslation(), boneAxisWorld)) {
        ...
        for (int i = 0; i < NUM_BASE_CORNERS * 2; i++) {
            v->pos = pose0.getTranslation();
            v->rgba = color;
            v++;
            v->pos = pose1.getTranslation();
            v->rgba = color;
            v++;
        }
    }
    ...
    pendingChanges.updateItem<AnimDebugDrawData>(_itemID, [&](AnimDebugDrawData& data) {
        ...
        for (auto& iter : _absolutePoses) {
            ...
            for (int i = 0; i < skeleton->getNumJoints(); i++) {
                const float radius = BONE_RADIUS / (absPoses[i].getScale().x * rootPose.getScale().x);
