...
void RenderableModelEntityItem::computeShapeInfo(ShapeInfo& shapeInfo) {
    ...
    ShapeType type = getShapeType();
    glm::vec3 halfExtents  = 0.5f * getDimensions();
    if (type == SHAPE_TYPE_COMPOUND) {
        ...
        shapeInfo.setParams(type, halfExtents, _compoundShapeURL);
        glm::vec3 invRegistrationOffset = -2.0f * halfExtents * (ENTITY_ITEM_DEFAULT_REGISTRATION_POINT - getRegistrationPoint());
        CollisionShapeUtil::computeShapeInfoFromGeometryResource(_compoundShapeResource, _model, invRegistrationOffset, shapeInfo);
    } else if (type >= SHAPE_TYPE_SIMPLE_HULL && type <= SHAPE_TYPE_STATIC_MESH) {
        ...
        shapeInfo.setParams(type, halfExtents, _modelURL);
        glm::vec3 invRegistrationOffset = -2.0f * halfExtents * (ENTITY_ITEM_DEFAULT_REGISTRATION_POINT - getRegistrationPoint());
        CollisionShapeUtil::computeShapeInfoFromModel(_model, invRegistrationOffset, shapeInfo);
    } else {
        ModelEntityItem::computeShapeInfo(shapeInfo);
        shapeInfo.setParams(type, halfExtents);
    }
