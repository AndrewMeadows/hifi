    ...
    if (_nextIndex == -1) {
        ...
        if (element) {
            ...
            if (!view.usesViewFrustum) {
                ...
                float distance = glm::distance(view.viewFrustum.getPosition(), element->getAACube().calcCenter()) + MIN_VISIBLE_DISTANCE;
                float apparentAngle = element->getAACube().getScale() / distance;
