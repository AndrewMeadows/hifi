    ...
    for (auto& targetVar : _targetVarVec) {
        if (targetVar.jointIndex == -1) {
            ...
            if (target.getType() != IKTarget::Type::Unknown) {
                ...
                glm::quat rotation = animVars.lookupRigToGeometry(targetVar.rotationVar, defaultPose.getRotation());
                glm::vec3 translation = animVars.lookupRigToGeometry(targetVar.positionVar, defaultPose.getTranslation());
...
void AnimInverseKinematics::solveWithCyclicCoordinateDescent(const std::vector<IKTarget>& targets) {
    ...
    while (maxError > MAX_ERROR_TOLERANCE && numLoops < MAX_IK_LOOPS) {
        ...
        for (int i = lowestMovedIndex; i < _maxTargetIndex; ++i) {
            if (_accumulators[i].size() > 0) {
                _relativePoses[i].setRotation(_accumulators[i].getAverage());
        ...
        for (size_t i = 0; i < targets.size(); i++) {
            if (targets[i].getType() == IKTarget::Type::RotationAndPosition || targets[i].getType() == IKTarget::Type::HmdHead ||
                ...
                float error = glm::length(absolutePoses[targets[i].getIndex()].getTranslation() - targets[i].getTranslation());
                if (error > maxError) {
                    maxError = error;
                }
            }
    ...
    for (auto& target: targets) {
        ...
        if (parentIndex != -1) {
            ...
            // Q = Qp * q   -->   q' = Qp^ * Q
            glm::quat newRelativeRotation = glm::inverse(absolutePoses[parentIndex].getRotation()) * targetRotation;
            ...
            _relativePoses[tipIndex].setRotation(newRelativeRotation);
            absolutePoses[tipIndex].setRotation(targetRotation);
        }
    }
}
...
int AnimInverseKinematics::solveTargetWithCCD(const IKTarget& target, AnimPoseVec& absolutePoses) {
    ...
    glm::quat tipOrientation = absolutePoses[tipIndex].getRotation();

    // also cache tip's parent's absolute orientation so we can recompute
    // the tip's parent-relative as we proceed up the chain
    glm::quat tipParentOrientation = absolutePoses[pivotIndex].getRotation();
    ...
    glm::vec3 tipPosition = absolutePoses[tipIndex].getTranslation();

    // descend toward root, pivoting each joint to get tip closer to target position
    while (pivotIndex != _hipsIndex && pivotsParentIndex != -1) {
        // compute the two lines that should be aligned
        glm::vec3 jointPosition = absolutePoses[pivotIndex].getTranslation();
        ...
        if (targetType == IKTarget::Type::RotationAndPosition ||
            ...
            if (constraint && constraint->isLowerSpine()) {
                ...
                glm::vec3 twistAxis = absolutePoses[pivotIndex].getTranslation() - absolutePoses[pivotsParentIndex].getTranslation();
        ...
        glm::quat newRot = glm::normalize(glm::inverse(
                absolutePoses[pivotsParentIndex].getRotation()) *
                deltaRotation *
                absolutePoses[pivotIndex].getRotation());
        ...
        if (constraint) {
            ...
            if (constrained) {
                ...
                // Q' = Qp^ * dQ * Q  -->  dQ =   Qp * Q' * Q^
                deltaRotation = absolutePoses[pivotsParentIndex].getRotation() * newRot * glm::inverse(absolutePoses[pivotIndex].getRotation());
            }
        }
...
const AnimPoseVec& AnimInverseKinematics::overlay(const AnimVariantMap& animVars, float dt, Triggers& triggersOut, const AnimPoseVec& underPoses) {
    ...
    if (_relativePoses.size() != underPoses.size()) {
        ...
        int numJoints = (int)_relativePoses.size();
        for (int i = 0; i < numJoints; ++i) {
            float dotSign = copysignf(1.0f, glm::dot(_relativePoses[i].getRotation(), underPoses[i].getRotation()));
            if (_accumulators[i].isDirty()) {
                // this joint is affected by IK --> blend toward underPose rotation
                _relativePoses[i].setRotation(glm::normalize(glm::lerp(_relativePoses[i].getRotation(), dotSign * underPoses[i].getRotation(), blend)));
            } else {
                // this joint is NOT affected by IK --> slam to underPose rotation
                _relativePoses[i].setRotation(underPoses[i].getRotation());
            }
            _relativePoses[i].getTranslation() = underPoses[i].getTranslation();
        }
        ...
        if (!_relativePoses.empty()) {
            ...
            while (constraintItr != _constraints.end()) {
                ...
                constraintItr->second->dynamicallyAdjustLimits(_relativePoses[index].getRotation());
                ++constraintItr;
            }
        }
    }
    ...
    if (!_relativePoses.empty()) {
        ...
        if (targets.empty()) {
            ...
            while (constraintItr != _constraints.end()) {
                int index = constraintItr->first;
                glm::quat rotation = _relativePoses[index].getRotation();
                constraintItr->second->apply(rotation);
                _relativePoses[index].setRotation(rotation);
                ++constraintItr;
            }
        } else {

            {
                ...
                if (offsetLength > MIN_HIPS_OFFSET_LENGTH && _hipsIndex >= 0) {
                    ...
                    if (_hipsParentIndex == -1) {
                        ...
                        _relativePoses[_hipsIndex].getTranslation() = underPoses[_hipsIndex].getTranslation() + scaleFactor * _hipsOffset;
                    } else {
                        // the hips are NOT the root so we need to transform _hipsOffset into hips local-frame
                        glm::quat hipsFrameRotation = _relativePoses[_hipsParentIndex].getRotation();
                        int index = _skeleton->getParentIndex(_hipsParentIndex);
                        while (index != -1) {
                            hipsFrameRotation *= _relativePoses[index].getRotation();
                            index = _skeleton->getParentIndex(index);
                        }
                        _relativePoses[_hipsIndex].getTranslation() = underPoses[_hipsIndex].getTranslation()
                ...
                for (auto& target: targets) {
                    ...
                    if (targetIndex == _headIndex && _headIndex != -1) {
                        ...
                        if (target.getType() == IKTarget::Type::RotationOnly) {
                            ...
                            glm::vec3 under = _skeleton->getAbsolutePose(_headIndex, underPoses).getTranslation();
                            glm::vec3 actual = _skeleton->getAbsolutePose(_headIndex, _relativePoses).getTranslation();
                        ...
                        glm::vec3 actualPosition = _skeleton->getAbsolutePose(targetIndex, _relativePoses).getTranslation();
...
void AnimInverseKinematics::initConstraints() {
    ...
    for (int i = 0; i < numJoints; ++i) {
        ...
        if (0 == baseName.compare("Arm", Qt::CaseSensitive)) {
            ...
            stConstraint->setReferenceRotation(_defaultRelativePoses[i].getRotation());
            ...
            glm::quat invAbsoluteRotation = glm::inverse(absolutePoses[i].getRotation());
            ...
            */
            ...
            // rotate directions into joint-frame
            glm::quat invRelativeRotation = glm::inverse(_defaultRelativePoses[i].getRotation());
            int numDirections = (int)swungDirections.size();
            for (int j = 0; j < numDirections; ++j) {
            ...
            */
            ...
            glm::quat referenceRotation = _defaultRelativePoses[i].getRotation();
