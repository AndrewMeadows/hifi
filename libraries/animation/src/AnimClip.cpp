        ...
        for (int animJoint = 0; animJoint < animJointCount; animJoint++) {
            ...
            if (skeletonJoint >= 0 && skeletonJoint < skeletonJointCount) {
                ...
                if (usePreAndPostPoseFromAnim) {
                    ...
                    preRot = AnimPose(glm::vec3(1.0f), _skeleton->getRelativeBindPose(skeletonJoint).getRotation(), glm::vec3());
                    postRot = AnimPose::identity;
                }

                // cancel out scale
                preRot.setScale(glm::vec3(1.0f));
                postRot.setScale(glm::vec3(1.0f));
                ...
                if (fabsf(glm::length(fbxZeroTrans)) > EPSILON) {
                    boneLengthScale = glm::length(relDefaultPose.getTranslation()) / glm::length(fbxZeroTrans);
                }

                AnimPose trans = AnimPose(glm::vec3(1.0f), glm::quat(), relDefaultPose.getTranslation() + boneLengthScale * (fbxAnimTrans - fbxZeroTrans));
