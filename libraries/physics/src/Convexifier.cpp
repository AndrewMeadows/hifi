...
// BOOKMARK TODO:
//
// * add vhacd lib dependency to interface
// * fix build
// * finish Convexifier

#include "Convexifier.h"

// concavity = max allowed curvature
// range = [0.0, 1.0]
const double VHACD_DEFAULT_CONCAVITY = 0.0025;

// alpha = bias toward clipping along symmetry
// range = [0.0, 1.0]
// default = 0.05
const double VHACD_DEFAULT_ALPHA = 0.05;

// beta = bias toward clipping along revolution
// range = [0.0, 1.0]
// default = 0.05
const double VHACD_DEFAULT_BETA = 0.05;

// gamma = max concavity allowed during merge stage
// range = [0.0, 1.0]
// default = 0.00125
const double VHACD_DEFAULT_GAMMA = 0.00125;

// delta = bias toward maximizing local concavity
// range = [0.0, 1.0]
// default = 0.05
const double VHACD_DEFAULT_DELTA = 0.05;

const double VHACD_DEFAULT_MIN_VOLUME_PER_HULL = 0.0001;

// resolution = max number of voxels generated during the voxelization stage
// range = [1e4, 1.6e7]
// default = 1e5
const uint32_t VHACD_DEFAULT_RESOLUTION = 1.0e5;

// vertsPerHull = max number of vertices in each hull
// range = [4, 1024]
// default = 42
const uint32_t VHACD_DEFAULT_MAX_NUM_VERTS_PER_HULL = 42;

// depth = max number of clipping stages
// range = [1, 32]
// default = 20
const int32_t VHACD_DEFAULT_DEPTH = 20;

// planeDownSampling = granularity of search for best clipping plane
// range = [1, 16]
// default = 4
const int32_t VHACD_DEFAULT_PLANE_DOWN_SAMPLING = 4;

// hullDownSampling = precision of hull generation during clipping
// range = [1, 16]
// default = 4
const int32_t VHACD_DEFAULT_HULL_DOWN_SAMPLING = 4;

// pca = enable/disable normalization of mesh before decomposition
const int32_t VHACD_DEFAULT_PCA = 0; // false

// mode = 0 for voxels, 1 for tetrahedron
const int32_t VHACD_DEFAULT_MODE = 0; // voxels

const int32_t VHACD_DEFAULT_HULL_APPROXIMATION = 1; // true

const int32_t VHACD_DEFAULT_OPENCL_ACCELERATION = 0; // false


// util method
void initDefaultVhacdParams(VhacdParameters& params) {
    params.m_concavity = VHACD_DEFAULT_CONCAVITY;
    params.m_alpha = VHACD_DEFAULT_ALPHA;
    params.m_beta = VHACD_DEFAULT_BETA;
    params.m_gamma = VHACD_DEFAULT_GAMMA;
    params.m_delta = VHACD_DEFAULT_DELTA;
    params.m_minVolumePerCH = VHACD_DEFAULT_MIN_VOLUME_PER_HULL;
	params.m_callback = nullptr;
	params.m_logger = nullptr;
    params.m_resolution = VHACD_DEFAULT_RESOLUTION;
    params.m_maxNumVerticesPerCH = VHACD_DEFAULT_MAX_NUM_VERTS_PER_HULL;
    params.m_depth = VHACD_DEFAULT_DEPTH;
    params.m_planeDownsampling = VHACD_DEFAULT_PLANE_DOWN_SAMPLING;
    params.m_convexhullDownsampling = VHACD_DEFAULT_HULL_DOWN_SAMPLING;
    params.m_pca = VHACD_DEFAULT_PCA;
    params.m_mode = VHACD_DEFAULT_MODE;
    params.m_convexhullApproximation = VHACD_DEFAULT_HULL_APPROXIMATION;
    params.m_oclAcceleration = VHACD_DEFAULT_OPENCL_ACCELERATION;
}

Convexifier::Convexifier(MeshWorker::Data& data) : MeshWorker(data) {
    initDefaultVhacdParams(_params);
}
